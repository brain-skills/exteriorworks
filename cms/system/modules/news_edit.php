<?php
    ...
    */
    ...
    $query_categories = "SELECT * FROM news_categories";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && $action == 'news_edit') {
        ...
        if (isset($_POST['page_id'])) {
            ...
            $query = "UPDATE news SET date = ?, categories = ?, title = ?, alt_name = ?, short_desc = ?, full_desc = ?, meta_keys = ?, meta_desc = ? WHERE id = ?";
            $stmt = mysqli_prepare($db_connect, $query);
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "ssssssssi", $date, $categories, $title, $alt_name, $short_desc, $full_desc, $meta_keys, $meta_desc, $page_id);
    ...
    } elseif ($_SERVER["REQUEST_METHOD"] == "GET" && $action == 'news_edit') {
    ...
    $yourData = array('title' => 'Редактирование новости - Skills Energy');
