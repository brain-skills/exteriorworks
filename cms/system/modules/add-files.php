<?php
    ...
    */
    ...
    $sql = "SELECT MAX(id) AS max_id FROM files";
    $result = $db_connect->query($sql);

    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
			$page_id = $row["max_id"] + 1;
            // $file_id = isset($row['max_id']) ? $row['max_id'] + 1 : 1;
            $max_id = $row["max_id"];
        }
    } else {
		$page_id = 1;
    ...
    $sql_reset_auto_increment = "ALTER TABLE files AUTO_INCREMENT = " . ($max_id + 1);
    $db_connect->query($sql_reset_auto_increment);

    // Получаем категории
    $categories = [];
    $sql = "SELECT id, parentid, name FROM files_categories ORDER BY name ASC";
    $result = $db_connect->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $categories[] = $row;
    ...
    if (isset($_POST['addfile'])) {
        ...
        // Выполняем запрос на добавление файла в базу данных
        $insertQuery = "INSERT INTO files (autor, date, categories, title, file_path, image, alt_name, short_desc, full_desc, price, version, license, contact_info, views) VALUES ('$autor', '$date', '$categories', '$title', '$file', '$image', '$alt_name', '$short_desc', '$full_desc', '$price', '$version', '$license', '$contact_info', '$views')";
        ...
        if ($result) {
            echo "Новый файл успешно добавлен!";
        } else {
            echo "Ошибка при добавлении файла: " . $db_connect->error;
        }
    ...
    $yourData = array('title' => $lang['nav']['addfiles'] . ' - Skills Energy');
