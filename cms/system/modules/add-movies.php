<?php
    ...
    */
    ...
    $sql_reset_auto_increment = "ALTER TABLE movies AUTO_INCREMENT = " . ($max_id + 1);
    $db_connect->query($sql_reset_auto_increment);

    // Получаем категории
    $categories = [];
    $sql = "SELECT id, parentid, name FROM movies_categories ORDER BY name DESC";
    $result = $db_connect->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $categories[] = $row;
    ...
    if (isset($_POST['addmovie'])) {
        ...
        $insertQuery = "INSERT INTO movies (autor, date, categories, title, image, alt_name, search_id, year, quality, director, actors, short_desc, full_desc, meta_desc, meta_keys, views) VALUES ('$autor', '$date', '$categories', '$title', '$image', '$alt_name', '$search_id', '$year', '$quality', '$director', '$actors', '$short_desc', '$full_desc', '$meta_desc', '$meta_keys', '$views')";
    ...
    $yourData = array('title' => $lang['nav']['addmovies'] . ' - Skills Energy');
